...
export default function Property({currentUser, property, handleUpgradeProperty}) {
    ...
    const lowestNotOwnedLevel = property.upgrades
        .find(upgrade => !upgrade.isOwned && upgrade.level.startsWith("LEVEL"));
    ...
    return (
        <div className={"property-color color-" + property.member.color + "-g"}>
            ...
            {property.upgrades.map((upgrade, index) => {
                ...
                const upgradeRequirement = property.upgradeRequirements
                    .find(upg => upg.level === upgrade.level);
                const hasFalseRequirement = upgradeRequirement ?
                    Object.values(upgradeRequirement.requirements).some(value => !value) : true;
                return (
                    <div key={index} className={"property-modifier-div " + (upgrade.isOwned ? "modifiered" : "") +
                        (hasFalseRequirement ? " property-div-compleated" : "")}>
                        <h3 className="property-modifier-h3">{propertyLevelInfo.name}</h3>
                        <div className="property-grid-3 ">
                            ...
                            {(() => {
                                return upgradeRequirement ?
                                    Object.entries(upgradeRequirement.requirements).map(([key, value]) => (
                                        requirements[key]
                            ...
                            <div className="property-new-stats">
                                ...
                                <div className="property-mini-flex">
                                    <p className="property-new-stats-p">g.p.t</p>
                                    <div className="player-stat-gold gold-per-turn width-full pointer no-select">
                                        <img
                                            src={goldImg}
                                            className="recourse-img"
            ...
            <div className=" proprty-btns-div flex-between">
                {lowestNotOwnedLevel ?
                    <button disabled={currentUser.gold < lowestNotOwnedLevel.price ||
                        (property.upgradeRequirements.length > 0 &&
                            property.upgradeRequirements.some(upg => upg.level === lowestNotOwnedLevel.level) &&
                            Object.values(property.upgradeRequirements
                                .find(upgrade => upgrade.level === lowestNotOwnedLevel.level).requirements)
                                .some(req => req === false))}
