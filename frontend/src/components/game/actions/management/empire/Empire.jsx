...
export default function Empire({currentUser, selectProperty, properties, handleUpgradeProperty}) {
    return (
        <div>
            {properties && Object.keys(properties).map((key) => {
                ...
                if (property.member && property.member.user.username === Cookies.get('username')) {
                    ...
                    const ownedLevels = property.upgrades.filter(upgrade => upgrade.isOwned &&
                    ...
                    return (
                        <div key={key} className={`property-color empire-component color-${property.member.color}-g`}>
                            <h2 onClick={() => selectProperty(property.position)} className="property-cell-name">
                                {propertyName}
                            </h2>
                            <div className="property-grid">
                                <div onClick={() => selectProperty(property.position)} className="property-img-div">
                                    <img
                                ...
                                {lowestNotOwnedLevel ?
                                    <button disabled={currentUser.gold < lowestNotOwnedLevel.price ||
                                        (property.upgradeRequirements.length > 0 &&
                                            property.upgradeRequirements
                                                .some(upg => upg.level === lowestNotOwnedLevel.level) &&
                                            Object.values(property.upgradeRequirements
                                                .find(upgrade =>
                                                    upgrade.level === lowestNotOwnedLevel.level).requirements)
                                                .some(req => req === false))}
